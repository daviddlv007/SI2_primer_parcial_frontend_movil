...
class VoiceCommandService {
  ...
  Future<void> processCommand(String text, BuildContext context) async {
    ...
    try {
      if (RegExp(r'crear carrito').hasMatch(command)) {
        ...
        r'agregar\s+(\d+|un|uno|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\s+(.+)',
      ).hasMatch(command)) {
  ...
  Future<void> _handleAddProduct(
    ...
    final match = RegExp(
      r'agregar\s+(\d+|un|uno|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\s+(.+)',
    ).firstMatch(command.toLowerCase());
  ...
  Future<void> _handleRemoveProduct(
    ...
    if (match == null || match.group(2) == null) {
      throw FormatException(
        'Formato incorrecto. Ejemplo: "eliminar iPhone 15"',
      );
    }
    ...
    try {
      ...
      print(
        '[DEBUG] Eliminando producto ID: ${product.id} del carrito ID: ${provider.selectedCartId}',
      );
  ...
  Future<void> _handleDeleteCart(
    BuildContext context,
    CartProvider provider,
  ) async {
    if (provider.selectedCartId == null)
      throw Exception('No hay carrito seleccionado');
